...
```{r hoboplot, fig.cap="Fifteen minute interval stream depth records at each site.", out.width="100%", fig.asp=1}
ggplot(hobo_df) +
...
```{r iqimport, fig.cap="Period of recorded data at each station."}
## make a list of files to import
...
Two National Oceanic and Atmospheric Administration (NOAA) Global Historical Climatology Network (GHCN) locations provide daily precipitation data for the project area. GHCN daily summaries were downloaded for GHCND:USW00003904 (Easterfield Airport) and GHCND:US1TXBZS088 (Bryan 3.5 NNW, TX US) using the NOAA API services and the `rnoaa` package in R [@chamberlain_2019].
...
easterfield_precip <- read_csv("Data/noaa_precip/easterfield.csv",
                               col_types = cols(
...
bryan_precip <- read_csv("Data/noaa_precip/bryan3_5nnw.csv",
                         col_types = cols(
                           date = col_datetime(format = ""),
                           datatype = col_character(),
                           station = col_character(),
...
easterfield_precip %>%
  ...
  mutate(date = as.Date(date),
         value = as.numeric(value)) %>%
  ggplot() +
  geom_histogram(aes(value), binwidth = 10) +
  facet_wrap(~station, ncol = 1) +
  coord_flip() +
  theme_ms() +
...
```{r metricplot16396, fig.cap="Scatter plot of rating curve estimated flows against measured flows (A, C) and stage discharge predictions (B, D) for each rating curve period at station 16396.", out.width="100%", fig.asp = 1}
...
```{r metricplot16397, fig.cap="Scatter plot of rating curve estimated flows against measured flows (A) and stage discharge predictions (B) for station 16397.", out.width="100%", fig.asp = .5}
## plot rating curve results
p1 <- df_16397 %>%
...
```{r metricplot16882, fig.cap="Scatter plot of rating curve estimated flows against measured flows (A, C, E) and stage discharge predictions (B, D, F) for each rating curve period at station 16882.", out.width="100%", fig.asp = 1}
## plot rating curve results
...
```{r instantresults, out.width="100%", fig.asp = 1, fig.cap="Fifteen minute streamflow estimates with overlaid measured streamflow values"}
...
```{r meandailyresults, out.width="100%", fig.asp = 1, fig.cap="Mean daily streamflow estimates."}
...
```{r message=FALSE}
read_csv("Data/USGS_Streamflow/08110100.csv") %>%
  filter(datetime >= as.Date("2020-03-03") & datetime <= as.Date("2021-01-14")) %>%
  rename(Flow = `135389_00060_00003`) -> mean_daily_08110100


read_csv("Data/USGS_Streamflow/08109800.csv") %>%
  filter(datetime >= as.Date("2020-03-03") & datetime <= as.Date("2021-01-14")) %>%
  rename(Flow = `135356_00060_00003`) -> mean_daily_08109800


read_csv("Data/USGS_Streamflow/08065800.csv") %>%
  filter(datetime >= as.Date("2020-03-03") & datetime <= as.Date("2021-01-14")) %>%
  rename(Flow = `133946_00060_00003`) -> mean_daily_08065800


mean_daily_flow %>%
  as_tibble() %>%
  filter(Site == "16396") %>%
  left_join(mean_daily_08110100 %>% select(datetime, Flow_08110100 = Flow), by = c("date" = "datetime")) %>%
  left_join(mean_daily_08109800 %>% select(datetime, Flow_08109800 = Flow), by = c("date" = "datetime")) %>%
  left_join(mean_daily_08065800 %>% select(datetime, Flow_08065800 = Flow), by = c("date" = "datetime")) -> model_df
```


```{r}
####
# next steps: calculate fdc for 2020 at each site
# regress at selected quantiles using watershed characteristics
# predict and linear interpolate to fill in. 
# use transfer procedure from candidate gaage. 
####
# evaluate DAR procedure as below, looks like we don't have a great candidate
# gage as the magnitude of the peaks are not in the correct order.
####
#evaluate a rainfall runoff mode? q~f(x,y) runoff is a function of precip and observed runoff?
# precip data vailable through rnoaa at easterfield which is very close by.
# 	GHCND:USW00003904
####
...
# ####
# area_16396 <- 24 ## need to adjust this, not quite right
# area_08110100 <- 195
# area_08109800 <- 244
# area_08065800 <- 321
# dar16396 <- formula(mean_daily ~ (Flow_08110100 * (24/195)^phi + 
#                       Flow_08109800 * (24/244)^upsilon +
#                       Flow_08065800 * (24/321)^tau)/3)
# 
# ## some starting paremeters. nls_multstart will use multiple
# ## starting parameters and model selection to find 
# ## global minimum
# start_lower <- list(phi = 0.1, upsilon = 0.1, tau = 0.1)
# start_upper <- list(phi = 10, upsilon = 10, tau = 10)
# ## fit nls
# multidar <- nls_multstart(dar16396,
#               data = model_df,
#               iter = 1000,
#               start_lower = start_lower,
#               start_upper = start_upper,
...
# 
# model_df %>%
#   mutate(fit = predict(multidar, newdata = .)) %>%
#   ggplot() +
#   geom_line(aes(date, mean_daily)) +
#   geom_point(aes(date, fit))
```


```{r}
### Use linear regression to estiamte streamflow response
### Models: 
### log(y) ~ easterwood + easterwood lag1 + bryan + bryan lag1;
```
